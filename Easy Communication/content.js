...
(function () {
    ...
    async function init() {
        try {
            const data = await chrome.storage.local.get(['imei_username', 'tracked_imeis', 'expanded_techs', 'target_url']);
    ...
    function attachEventListeners() {
        ...
        if (toggleBtn) toggleBtn.onclick = () => {
            ...
            document.body.classList.toggle('imei-panel-open', !container.classList.contains('collapsed'));
        };

        if (closeBtn) closeBtn.onclick = () => {
            document.getElementById('imei-panel-container').classList.add('collapsed');
    ...
    function startTracking() {
        ...
        trackingInterval = setInterval(() => {
            ...
            const userInput = document.getElementById('CagriMerkeziKayitGetirDto_KayitKullanicisi');

            if (!imeiInput || !userInput) {
            ...
            const pageUser = userInput.value ? userInput.value.trim() : '';
    ...
    function switchTab(tabName) {
        ...
        if (chatPage) chatPage.style.display = isChat ? 'flex' : 'none';
        if (qtyPage) qtyPage.style.display = isChat ? 'none' : 'flex';
    ...
    function showPage(page) {
        ...
        document.querySelectorAll('.page-container').forEach(p => {
            ...
            p.style.display = 'none'; // Flex/None çakışmasını önlemek için
        });
        ...
        if (active) {
            ...
            // Chat sayfası için flex, diğerleri için css class'ı yeterli olabilir ama burada garantiye alalım
            if (page === 'login') active.style.display = 'flex';
        }
    }
    ...
    function renderMessages() {
        ...
        if (msgs.length === 0) {
            list.innerHTML = `<div class="empty-state">${icons.chat}<p>Sonuç bulunamadı</p></div>`;
            return;
        }
